...
class User(Base):
    ...
    permissions = Column(JSON, default=dict)  # 权限配置，存储可访问的端点列表
...
class RefreshToken(Base):
    """Refresh Token 表：支持 Token 刷新和主动失效"""
...
class MaterialInventory(Base):
    ...
    certificate_file = Column(String(255))      # 质检报告文件名
    production_date = Column(String(8))        # 生产日期 YYYYMMDD
    expiration_date = Column(String(8))        # 有效期 YYYYMMDD
    status = Column(String(20), default='active')  # active(可用)/depleted(已用完)
