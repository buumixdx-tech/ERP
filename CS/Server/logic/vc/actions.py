...
def create_procurement_vc_action(session: Session, payload: CreateProcurementVCSchema, draft_rules: list = None) -> ActionResult:
    ...
    try:
        with audit_context("create_procurement_vc_action"):
            ...
            evt = emit_event(session, SystemEventType.VC_CREATED, SystemAggregateType.VIRTUAL_CONTRACT, 0, {"placeholder": True})
            ...
            evt.aggregate_id = new_vc.id
            evt.payload = {"type": new_vc.type, "business_id": new_vc.business_id, "total_amount": payload.total_amt}
...
def create_material_supply_vc_action(session: Session, payload: CreateMaterialSupplyVCSchema, draft_rules: list = None) -> ActionResult:
    """物料供应执行单创建 Action"""
    try:
        with audit_context("create_material_supply_vc_action"):
            biz = session.query(Business).get(payload.business_id)
            if not biz: return ActionResult(success=False, error="未找到关联业务项目")
            if biz.status not in [BusinessStatus.ACTIVE, BusinessStatus.LANDING]:
...
def create_return_vc_action(session: Session, payload: CreateReturnVCSchema, draft_rules: list = None) -> ActionResult:
    ...
    """
    try:
        with audit_context("create_return_vc_action"):
...
def update_vc_action(session: Session, payload: UpdateVCSchema) -> ActionResult:
    """底层 VC 数据修正 Action"""
    try:
        with audit_context("update_vc_action"):
            ...
            evt = emit_event(session, SystemEventType.VC_UPDATED, SystemAggregateType.VIRTUAL_CONTRACT, 0, {"placeholder": True})
            set_audit_event_id(evt.id)

            evt.aggregate_id = vc.id
            evt.payload = {"desc": vc.description}
...
def delete_vc_action(session: Session, payload: DeleteVCSchema) -> ActionResult:
    ...
    try:
        with audit_context("delete_vc_action"):
            ...
            evt = emit_event(session, SystemEventType.VC_DELETED, SystemAggregateType.VIRTUAL_CONTRACT, 0, {"placeholder": True})
            set_audit_event_id(evt.id)

            session.delete(vc)

            evt.aggregate_id = vc_id
            evt.payload = None
...
def create_mat_procurement_vc_action(session: Session, payload: CreateMatProcurementVCSchema, draft_rules: list = None) -> ActionResult:
    """物料采购执行单创建 Action"""
    try:
        with audit_context("create_mat_procurement_vc_action"):
            sc = session.query(SupplyChain).get(payload.sc_id)
            ...
            evt.aggregate_id = new_vc.id
            evt.payload = {"type": new_vc.type, "total_amount": payload.total_amt}
...
def create_stock_procurement_vc_action(session: Session, payload: CreateStockProcurementVCSchema, draft_rules: list = None) -> ActionResult:
    """库存采购执行单创建 Action"""
    try:
        with audit_context("create_stock_procurement_vc_action"):
            sc = session.query(SupplyChain).get(payload.sc_id)
...
def create_inventory_allocation_action(session: Session, payload: AllocateInventorySchema) -> ActionResult:
    """库存拨付 Action"""
    try:
        with audit_context("create_inventory_allocation_action"):
            biz = session.query(Business).get(payload.business_id)
            if not biz: return ActionResult(success=False, error="未找到关联业务项目")
            ...
            # 收集所有将被修改的 EquipmentInventory SN
            all_sn_list = []
            for e in payload.elements:
            ...
            evt.aggregate_id = new_vc.id
            evt.payload = {"type": new_vc.type, "equipment_count": sum(e.qty for e in payload.elements)}
