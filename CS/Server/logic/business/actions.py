...
def create_business_action(session: Session, payload: CreateBusinessSchema) -> ActionResult:
    try:
        with audit_context("create_business_action"):
            new_b = Business(
            ...
            session.add(new_b)
            session.flush()
            evt = emit_event(session, SystemEventType.BUSINESS_CREATED, SystemAggregateType.BUSINESS, 0, {"placeholder": True})
            set_audit_event_id(evt.id)
            evt.aggregate_id = new_b.id
            evt.payload = {"customer_id": payload.customer_id}
            session.commit()
...
def update_business_status_action(session: Session, payload: UpdateBusinessStatusSchema) -> ActionResult:
    try:
        with audit_context("update_business_status_action"):
            ...
            evt = emit_event(session, SystemEventType.BUSINESS_STATUS_CHANGED, SystemAggregateType.BUSINESS, 0, {"placeholder": True})
            set_audit_event_id(evt.id)
            RuleManager(session).propagate_from_parent(TimeRuleRelatedType.BUSINESS, biz.id)

            evt.aggregate_id = biz.id
            evt.payload = {"from": old_status, "to": payload.status}
...
def delete_business_action(session: Session, business_id: int) -> ActionResult:
    try:
        with audit_context("delete_business_action"):
            ...
            evt = emit_event(session, SystemEventType.BUSINESS_DELETED, SystemAggregateType.BUSINESS, 0, {"placeholder": True})
            set_audit_event_id(evt.id)

            session.delete(biz)

            evt.aggregate_id = business_id
            evt.payload = None
...
def advance_business_stage_action(session: Session, payload: AdvanceBusinessStageSchema) -> ActionResult:
    try:
        with audit_context("advance_business_stage_action"):
            biz = session.query(Business).get(payload.business_id)
            if not biz:
                raise_not_found_error("业务", str(payload.business_id))
            ...
            evt = emit_event(session, SystemEventType.BUSINESS_STAGE_ADVANCED, SystemAggregateType.BUSINESS, 0, {"placeholder": True})
            set_audit_event_id(evt.id)
            ...
            evt.aggregate_id = biz.id
            evt.payload = {"from": old_status, "to": payload.next_status}
