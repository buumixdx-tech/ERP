...
from logic.constants import LogisticsStatus, TimeRuleRelatedType, SystemEventType, SystemAggregateType, VCStatus
...
def confirm_inbound_action(session: Session, payload: ConfirmInboundSchema) -> ActionResult:
    """确认收货/入库 Action"""
    try:
        if not payload.sn_list:
            return ActionResult(success=False, error="序列号列表不能为空")
        
        if len(payload.sn_list) != len(set(payload.sn_list)):
